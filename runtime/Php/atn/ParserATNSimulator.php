...
Set = Utils::Set;
BitSet = Utils::BitSet;
DoubleDict = Utils::DoubleDict;
use Antlr4\ATN; //('./ATN').ATN;
use Antlr4\ATNState; //('./ATNState').ATNState;
...
use Antlr4\PredPrediction; //('./../dfa/DFAState').PredPrediction;
use Antlr4\ATNSimulator; //('./ATNSimulator').ATNSimulator;
use Antlr4\PredictionMode; //('./PredictionMode').PredictionMode;
use Antlr4\RuleContext; //('./../RuleContext').RuleContext;
use Antlr4\ParserRuleContext; //('./../ParserRuleContext').ParserRuleContext;
use Antlr4\SemanticContext; //('./SemanticContext').SemanticContext;
use Antlr4\StarLoopEntryState; //('./ATNState').StarLoopEntryState;
...
use Antlr4\NoViableAltException; //('./../error/Errors').NoViableAltException;
...
/* ParserATNSimulator */function reset() 
{
};

/* ParserATNSimulator */function adaptivePredict($input, $decision, $outerContext) 
{
...
// parser precedence, and is provided by a DFA method.
        ...
        if ($s0===null) 
        {
            ...
            $fullCtx = false;
            $s0_closure = $this->computeStartState($dfa->atnStartState, RuleContext::EMPTY, $fullCtx);
...
//    conflict + preds
//
/* ParserATNSimulator */function execATN($dfa, $s0, $input, $startIndex, $outerContext ) 
{
    if ($this->debug || $this->debug_list_atn_decisions) 
    {
...
// already cached
//
/* ParserATNSimulator */function getExistingTargetState($previousD, $t) 
{
...
// returns {@link //ERROR}.
//
/* ParserATNSimulator */function computeTargetState($dfa, $previousD, $t) 
{
...
/* ParserATNSimulator */function predicateDFAState($dfaState, $decisionState) 
...
/* ParserATNSimulator */function execATNWithFullContext($dfa, D,// how far we got before failing over
                                     $s0,
                                     $input,
                                     $startIndex,
                                     $outerContext) 
...
/* ParserATNSimulator */function computeReachSet($closure, $t, $fullCtx) 
{
    if ($this->debug) 
    {
...
// First figure out where we can reach on input t
    for ($i=0; $i<$closure->items->length;$i++) 
    {
        ...
        for($j=0;$j<$c->state->transitions->length;$j++) 
        {
            $trans = $c->state->transitions[$j];
            $target = $this->getReachableTarget($trans, $t);
            if ($target!==null) 
            {
                $cfg = new ATNConfig({$state:$target}, $c);
...
//
    if ($reach===null) 
    {
        ...
        for ($k=0; $k<$intermediate->items->length;$k++) 
        {
...
//
    ...
    {
        for ($l=0; $l<$skippedStopStates->length;$l++) 
        {
...
//
/* ParserATNSimulator */function removeAllConfigsNotInRuleStopState($configs, $lookToEndOfRule) 
{
...
/* ParserATNSimulator */function computeStartState($p, $ctx, $fullCtx) 
{// always at least the implicit call to start rule
    $initialContext = predictionContextFromRuleContext($this->atn, $ctx);
    $configs = new ATNConfigSet($fullCtx);
    for($i=0;$i<$p->transitions->length;$i++) 
    {
        $target = $p->transitions[$i].$target;
        $c = new ATNConfig({ $state:$target, $alt:$i+1, $context:$initialContext }, null);
        $closureBusy = new Set();
...
//
/* ParserATNSimulator */function applyPrecedenceFilter($configs) 
{
	$config;
	$statesFromAlt1 = [];
...
/* ParserATNSimulator */function getReachableTarget($trans, $ttype) 
{
...
/* ParserATNSimulator */function getPredsForAmbigAlts($ambigAlts, $configs, $nalts) 
...
/* ParserATNSimulator */function getPredicatePredictions($ambigAlts, $altToPred) 
{
    $pairs = [];
...
//
/* ParserATNSimulator */function getSynValidOrSemInvalidAltThatFinishedDecisionEntryRule($configs, $outerContext) 
{
...
/* ParserATNSimulator */function getAltThatFinishedDecisionEntryRule($configs) 
{
    $alts = [];
    for($i=0;$i<$configs->items->length; $i++) 
    {
...
//
/* ParserATNSimulator */function splitAccordingToSemanticValidity( $configs, $outerContext) 
{
...
//
/* ParserATNSimulator */function evalSemanticContext($predPredictions, $outerContext, $complete) 
{
    $predictions = new BitSet();
    for($i=0;$i<$predPredictions->length;$i++) 
    {
...
/* ParserATNSimulator */function closure($config, $configs, $closureBusy, $collectPredicates, $fullCtx, $treatEofAsEpsilon) 
{
    $initialDepth = 0;
...
/* ParserATNSimulator */function closureCheckingStopState($config, $configs, $closureBusy, $collectPredicates, $fullCtx, $depth, $treatEofAsEpsilon) 
{
...
// run thru all possible stack tops in ctx
        if (! $config->context->isEmpty()) 
        {
            for ( $i =0; $i<$config->context->length; $i++) 
            {
                ...
                $parms = {$state:$returnState, $alt:$config->alt, $context:$newContext, $semanticContext:$config->semanticContext};
...
/* ParserATNSimulator */function closure_($config, $configs, $closureBusy, $collectPredicates, $fullCtx, $depth, $treatEofAsEpsilon) 
{
    $p = $config->state;
// optimization
...
/* ParserATNSimulator */function canDropLoopEntryEdgeInLeftRecursiveRule($config) 
{// return False
    $p = $config->state;
...
/* ParserATNSimulator */function getRuleName( $index) 
{
...
/* ParserATNSimulator */function getEpsilonTarget($config, $t, $collectPredicates, $inContext, $fullCtx, $treatEofAsEpsilon) 
{
...
/* ParserATNSimulator */function actionTransition($config, $t) 
{
    if ($this->debug) 
    {
...
/* ParserATNSimulator */function precedenceTransition($config, $pt,  $collectPredicates, $inContext, $fullCtx) 
{
    if ($this->debug) 
    {
...
/* ParserATNSimulator */function predTransition($config, $pt, $collectPredicates, $inContext, $fullCtx) 
{
    if ($this->debug) 
    {
...
/* ParserATNSimulator */function ruleTransition($config, $t) 
{
    if ($this->debug) 
    {
...
/* ParserATNSimulator */function getConflictingAlts($configs) 
{
...
/* ParserATNSimulator */function getConflictingAltsOrUniqueAlt($configs) 
{
    $conflictingAlts = null;
...
/* ParserATNSimulator */function getTokenName( $t) 
{
    if ($t===Token::EOF) 
    {
        return "EOF";
...
/* ParserATNSimulator */function getLookaheadName($input) 
{
...
//  "dead" code for a bit.
//
/* ParserATNSimulator */function dumpDeadEndConfigs($nvae) 
{
...
/* ParserATNSimulator */function noViableAlt($input, $outerContext, $configs, $startIndex) 
{
    return new NoViableAltException($this->parser, $input, $input->get($startIndex), $input->LT(1), $configs, $outerContext);
};

/* ParserATNSimulator */function getUniqueAlt($configs) 
{
...
// on {@code to}
//
/* ParserATNSimulator */function addDFAEdge($dfa, $from_, $t, $to) 
{
    if( $this->debug) 
    {
    ...
    if ($this->debug) 
    {
        $literalNames = $this->parser===null ? null : $this->parser->literalNames;
        $symbolicNames = $this->parser===null ? null : $this->parser->symbolicNames;
...
//
/* ParserATNSimulator */function addDFAState($dfa, D) 
{
    if (D == ATNSimulator::ERROR) 
    {
        return D;
...
/* ParserATNSimulator */function reportAttemptingFullContext($dfa, $conflictingAlts, $configs, $startIndex, $stopIndex) 
{
...
/* ParserATNSimulator */function reportContextSensitivity($dfa, $prediction, $configs, $startIndex, $stopIndex) 
{
    if ($this->debug || $this->retry_debug) 
    {
        $interval = new Interval($startIndex, $stopIndex + 1);
...
/* ParserATNSimulator */function reportAmbiguity($dfa, D, $startIndex, $stopIndex,
